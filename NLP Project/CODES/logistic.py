...
for ll in range(5):
    ...
    lr = LogisticRegression( random_state=0,penalty='l2',multi_class='ovr',solver='liblinear',tol=0.01)
    model=lr.fit(OBS[ll],n1)
    ypred=lr.predict(observations2)

    n2ov.extend(n2)
    ypredov.extend(ypred)
    accuracy=model.score(observations2,n2)
    s+=accuracy
...
print ("avg f1 score logistic regression ovr: ",s/5)